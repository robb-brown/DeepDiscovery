...
def uNet(input,filterPlan,filterSize=(5,5),maxpool=False,layerThickness=1,dropout=None,normalization=None,activation=tf.nn.relu,name='UNet',dimensions=3):
	...
	logger.debug('{} Input'.format(name))
	net = input; #net.name = 'UInput'; downLayers.append(net)		ROBB - not sure why these are here?
	logger.debug('{} Down'.format(name))
	...
	for level,nFilters in enumerate(filterPlan):
		...
		if not dropout is None:
			net = tf.layers.dropout(net,rate=dropout,name='{}-DownDropout{}'.format(name,level+1),data_format='channels_last')
		if maxpool:
			net = maxpoolF(	inputs=net,
							...
							padding='same',
							data_format='channels_last',
							name='{}-DownMaxpool{}'.format(name,level+1),)
	...
	for index in range(0,len(filterPlan)):
		...
		if not normalization is None:
			pass
		if not dropout is None:
			net = tf.layers.dropout(net,rate=dropout,name='{}-UpDropout{}'.format(name,index+1),data_format='channels_last')
...
class UNet2D(Net):
	...
	def __init__(self,dimensions=(None,None,1),filterPlan=[10,20,30,40,50],filterSize=(5,5),layerThickness=1,postUDepth=2,maxpool=False,normalization=None,nonlinearity=tf.nn.relu,inputDropout=False,inputNoise=False,internalDropout=False,gentleCoding=0.9,name=None,**args):
		self.name = name if name is not None else 'UNet2D'
	...
	def createModel(self,dimensions=(None,None,None,1),filterPlan=[10,20,30,40,50],filterSize=(5,5),layerThickness=1,postUDepth=2,postUFilterSize=None,maxpool=False,inputDropout=False,inputNoise=False,internalDropout=False,**extras):
		...
		net = uNet(net,filterPlan = filterPlan,filterSize = filterSize,maxpool=maxpool,layerThickness=layerThickness,normalization=self.normalization,dimensions=2)
		for i in range(postUDepth):
			...
			if self.normalization is not None:
				pass
			if self.internalDropout:
				net = tf.layers.dropout(net,rate=self.internalDropoutProbability,name='{} PostUDropout{}'.format(name,i+1),data_format='channels_last')
